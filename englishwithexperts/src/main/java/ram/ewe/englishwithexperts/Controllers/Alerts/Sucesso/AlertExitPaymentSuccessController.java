...
public class AlertExitPaymentSuccessController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ModelControllers.getInstance().getAlertExitPaymentRegistrationController().hideDash();
        if(LanguagesRepository.getPort())
        {
            title.setText("Mensagem de Sucesso");
            info1.setText("Pagamento cancelado com sucesso.");
        }
        else
        {
            title.setText("Success Message");
            info1.setText("Payment successfully canceled.");
        }
    ...
    public void btnOK()
    {
        ModelControllers.getInstance().getUsersChangePasswordController().hideDash();
